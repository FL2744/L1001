...
- OpenAI gpt-5.4-nano by default, with optional OpenAI-compatible endpoints.
...
By default, the script reads OPENAI_API_KEY from ~/.env or the environment.
...
Provide an OpenAI API key using ONE of these methods:

    A. Let the script prompt for it when it starts; or
    B. Set it for the current Terminal session:
       export OPENAI_API_KEY="your-api-key-here"
    C. Store it in ~/.env as this single line:
...
def prompt_api_settings() -> tuple[str, str, str]:
    ...
    if choice == "1":
        ...
        if prompt_yes_no("Enter an API key now instead of using OPENAI_API_KEY?", False):
            api_key = getpass.getpass("OpenAI API key (input hidden): ").strip()
            if not api_key:
                print("No key entered; OPENAI_API_KEY will be used.")
        else:
            api_key = ""
    ...
    if choice == "2":
        ...
        arc_models = {
            "1": "gpt-oss-120b",
            "2": "gpt-oss-120b-thinking-low",
            "3": "gpt-oss-120b-thinking-high",
            "4": "GLM-5.2",
            "5": "GLM-5.2-non-thinking",
            "6": "Kimi-K3",
            "7": "DeepSeek-V4-Flash",
        }
        print("\nARC model:")
        for number, name in arc_models.items():
            ...
            print(f"  {number}. {name}{default_marker}")
        print("  8. Enter another ARC model ID")
        while True:
    ...
    while True:
        base_url = input(
            "Endpoint base URL (for example https://host.example/v1): "
        ).strip().rstrip("/")
        if base_url.startswith(("https://", "http://")):
            break
        print("Enter a full URL beginning with https:// or http://.")

    model = prompt_required("Model name/ID: ")
    api_key = getpass.getpass(
        "API key (input hidden; press Enter if the endpoint needs none): "
    ).strip()
    return model, base_url, api_key
...
def main() -> int:
    ...
    print(
        "  API key: "
        + ("entered interactively" if API_KEY_OVERRIDE else "environment/default")
    )
    print()
